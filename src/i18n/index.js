...
export const SUPPORTED_LOCALES = [
  ...
  { code: 'fa-IR', label: 'فارسی', dir: 'rtl' }
];

const localeDirs = Object.fromEntries(SUPPORTED_LOCALES.map((item) => [item.code, item.dir || 'ltr']));
...
export function createI18n(options = {}) {
  ...
  const translate = (key, params = {}, forcedLocale) => {
    ...
    const resolved = lookupMessage(targetLocale, key)
      ?? lookupMessage('en-US', key)
      ?? lookupMessage(DEFAULT_LOCALE, key);
  ...
  const set = (nextLocale) => {
    ...
    if (typeof document !== 'undefined') {
      ...
      document.documentElement.dir = localeDirs[normalized] || 'ltr';
    }
  };
  ...
  const instance = {
    ...
    install(app) {
      ...
      if (typeof document !== 'undefined') {
        ...
        document.documentElement.dir = localeDirs[locale.value] || 'ltr';
      }
    }
  };
